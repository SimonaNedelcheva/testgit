...
Nx = 20
Ny = 20
xc = np.arange(-lx/2+sigma_surround, lx/2, (lx-2*(sigma_surround))/(Nx-1))
yc = np.arange(-ly/2+sigma_surround, ly/2, (ly-2*(sigma_surround))/(Ny-1))
print xc
print yc
print xc+dx/5
print yc+dy/5
...
kernelON=np.zeros((Nx*Ny,kernel_duration,int(lx/dx),int(ly/dy)))
kernelOFF=np.zeros((Nx*Ny,kernel_duration,int(lx/dx),int(ly/dy)))

for xi in i:
	for yj in j:
		print 'kernel at', xi, yj
		# Create the kernel 
		kernelOFF[n,...] = create_kernel(dx, lx, dy, ly, sigma_surround, sigma_center, 
			dt_kernel, kernel_size, inverse=-1, x_tra=xc[xi], y_tra=yc[yj])
		kernelON[n,...] = create_kernel(dx, lx, dy, ly, sigma_surround, sigma_center, 
			dt_kernel, kernel_size, inverse=1, x_tra=xc[xi]+dx/5, y_tra=yc[yj]+dy/5)
		'''plt.figure(5)
		plt.subplot(2,2,1)	
		...
		for index in signal_indexes:
		    firing_rate_ON[index,n] = convolution(index, kernel_times, delay_indexes, stimuli_indexes, kernelON[n,...], stimuli)
		    firing_rate_OFF[index,n] = convolution(index, kernel_times, delay_indexes, stimuli_indexes, kernelOFF[n,...], stimuli)
